...
from .inference.constants import DEV_AI_CLOUD_LIST_MODELS, DEV_AI_CLOUD_LIST_ENDPOINTS, PROD_AI_CLOUD_LIST_MODELS, PROD_AI_CLOUD_LIST_ENDPOINTS, USER_ID
...
class Storage:
    def __init__(self, models_json_path: str = None, env_file_path: str = None):
        ...
        self.AI_CLOUD_LIST_ENDPOINTS = None
        self.AI_CLOUD_LIST_MODELS = ""

        # Read 'env' from os.environ, default to 'dev' if not set
        self.env = os.getenv('ENV', 'dev')
        self.USER_ID = os.getenv('USER_ID', USER_ID)

        if(self.env == 'dev'):
            self.AI_CLOUD_LIST_ENDPOINTS = DEV_AI_CLOUD_LIST_ENDPOINTS
            self.AI_CLOUD_LIST_MODELS = DEV_AI_CLOUD_LIST_MODELS
        else:
            self.AI_CLOUD_LIST_ENDPOINTS = PROD_AI_CLOUD_LIST_ENDPOINTS
            self.AI_CLOUD_LIST_MODELS = PROD_AI_CLOUD_LIST_MODELS
        ...
        for provider_name, provider in self.models_json.items():
            models = [
                Model(
                    ...
                    tasktype=model.get("tasktype", None),
                    endpoint=model.get("endpoint", None),
    ...
    def get_model_endpoints(self):
        """
        Fetches the list of model endpoints.
        """

        url = self.AI_CLOUD_LIST_ENDPOINTS
        headers= {
            'accept': 'application/json',
            'userId': self.USER_ID
        }
        response = requests.request("GET", url, headers=headers)
        endpoint_api_data = response.json()

        # logger.info(f"Endpoint API data: {endpoint_api_data}")
        return endpoint_api_data


    def get_models_from_api(self):
        """ 
        Fetches the models from ai cloud.S
        """
        url = self.AI_CLOUD_LIST_MODELS
        headers = {
        'accept': 'application/json',
        'userId': self.USER_ID
        }
        response = requests.request("GET", url, headers=headers)
        original_api_data = response.json()

        endpoint_data = self.get_model_endpoints()

        self.model_metadata = original_api_data

        # transformed_data = self.transform_json(dummy_data["data"],endpoint_data["data"])
        transformed_data = self.transform_json(original_api_data["data"],endpoint_data["data"])
        # logger.info(f"Transformed data: {transformed_data}")
        return transformed_data

    
    def transform_json(self,model_json_data, endpoint_json_data):
        """
        Transforms a JSON response with models grouped by providers.

        Args:
            model_json_data (dict): The original JSON data with model details.
            endpoint_json_data (dict): The JSON data with model endpoints.

        Returns:
            dict: The transformed JSON data with models grouped by tags.
        """

        endpoints_list = endpoint_json_data["endpoints"]
        models_list = model_json_data["models"]
        transformed_data = {}

        for model in models_list:
            tag = model["metadata"]["modelDetails"]["customTags"][1]["tags"].lower()

            # maps the endpoint based on model version and id
            for endpoint in endpoints_list:
                # logger.info(f"Model: {model['id']} Endpoint: {endpoint}")
                if "deployedModels" in endpoint:
                    for deployment in endpoint["deployedModels"]:
                        if model["id"] == deployment["modelId"] and model["version"] == deployment["version"]:
                            endpoint_uri = deployment["endpointUri"][0]
                            version = deployment["version"]
                            break

            if tag not in transformed_data:
                transformed_data[tag] = {"models": {},
                                         "requiresAPIKey": False,
                                         "remoteInference": True,
                                         "defaultParameters": None,
                                         "searchURL": None}

            # Check if model name is 'Mistral-7b' and adjust parameters accordingly
            # if model["name"] == 'Mistral-7b':
            #     parameters = {
            #         "max_new_tokens": {"value": 50, "range": [0, 300]},
            #         "temperature": {"value": 0.5, "range": [0.1, 1]},
            #         }
            # else:
            #     parameters = {
            #         "max_new_tokens": {"value": 50, "range": [0, 200]},
            #         "temperature": {"value": 0.5, "range": [0.1, 1]},
            ...
            modelParameters = model["metadata"]["modelParameters"]
            parameters = {}
            
            # Check if 'inputParameterMap' exists in modelParameters, use an empty list as default if not
            inputParameterMap = modelParameters.get("inputParameterMap", [])
            
            for inputParameter in inputParameterMap:
                parameters[inputParameter["key"]] = {
                    "value": inputParameter["defaultValue"],
                    "range": [inputParameter["lowerLimit"], inputParameter["upperLimit"]],
                }

            # Assuming modelParameters is a dictionary that may contain 'requestTemplate' as another dictionary
            raw_template = modelParameters.get("requestTemplate", None)
            
            # Check if raw_template is a dictionary, and if so, convert it to a JSON string
            if isinstance(raw_template, dict):
                requestTemplate = json.dumps(raw_template)
            else:
                requestTemplate = raw_template  # Use it directly if it's already a string or None
            transformed_data[tag]["models"][model["name"]] = {
                "metadata": {
                    "displayName": model["metadata"]["modelDetails"]["displayName"],
                    "endpoint": endpoint_uri,
                    "inputFormatMap": modelParameters["inputFormatMap"],
                    "requestTemplate": requestTemplate,
                    "outputFormatMap": modelParameters["outputFormatMap"],
                    "outputFormat": modelParameters["outputFormat"],
                },
                "endpoint": endpoint_uri,
                "tasktype": model["metadata"]["modelDetails"]["tasktype"].lower(),
                "capabilities": [],
                "enabled": True,
                "status": "ready",
    ...
    #     models_from_api = self.get_models_from_api()

    #     # printing the response of api
    #     logger.info(f"printing the response of api{models_from_api}")
    ...
    #     original_models_json_str = json.dumps(models_from_api)

    #     if not os.path.exists(os.path.join(APP_DIR, 'models.json')):
    #         with open(os.path.join(APP_DIR, 'models.json'), 'w') as f:
    #             f.write(original_models_json_str)
    #     else:
    #         # Convert the string back to a dictionary
    #         original_models_json = json.loads(original_models_json_str)

    #         with open(os.path.join(APP_DIR, 'models.json'), 'r') as f:
    #             cached_models_json = json.load(f)

    #             cached_providers = set(cached_models_json.keys())
    #             original_providers = set(original_models_json.keys())

    #             for provider in original_providers - cached_providers:
    #                 cached_models_json[provider] = original_models_json[provider]

    #             for provider in cached_providers & original_providers:
    #                 cached_provider_keys = set(cached_models_json[provider].keys())
    #                 original_provider_keys = set(original_models_json[provider].keys())

    #                 for key in cached_provider_keys - original_provider_keys:
    #                     del cached_models_json[provider][key]

    #                 for key in original_provider_keys - cached_provider_keys:
    #                     cached_models_json[provider][key] = original_models_json[provider][key]

    #                 cached_provider_models = set(cached_models_json[provider]['models'].keys())
    #                 original_provider_models = set(original_models_json[provider]['models'].keys())

    #                 for model in cached_provider_models & original_provider_models:
    #                     cached_model_keys = set(cached_models_json[provider]['models'][model].keys())
    #                     original_model_keys = set(original_models_json[provider]['models'][model].keys())

    #                     for key in original_model_keys - cached_model_keys:
    #                         cached_models_json[provider]['models'][model][key] = original_models_json[provider]['models'][model][key]

    #         # Write the updated cached_models_json back to the file
    #         with open(os.path.join(APP_DIR, 'models.json'), 'w') as f:
    #             json.dump(cached_models_json, f)
    ...
    def __initialize_config__(self, models_json_path: str = None):
        """
        Initializes the config file with the models from the API
        Args: 
            models_json_path (str): The path to the models.json file in case we want to load it from a specific location
        Returns:

        """
        models_from_api = self.get_models_from_api()
        return models_from_api, models_json_path  
