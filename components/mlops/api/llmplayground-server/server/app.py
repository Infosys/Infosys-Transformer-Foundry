...
class GlobalStateManager:
    ...
    def text_generation(self, inference_request: InferenceRequest):
        ...
        # logger.info(f"{self} provider in text_generation here is ------------------- {provider}")
        provider_details = ProviderDetails(
            api_key=None if inference_request.model_provider in ["aicloud", "azure", "openai"] else provider.api_key,
            version_key=None
        )
        # logger.info(f"Received inference request {inference_request}")

        if inference_request.model_provider == "azure":
            return self.inference_manager.azure_openai_text_generation(provider_details, inference_request)
        elif inference_request.model_provider == "aicloud":
            return self.inference_manager.aicloud_text_generation(provider_details, inference_request)
...
@click.option('--host',  '-H', default='0.0.0.0', help='The host to bind to. Default: 0.0.0.0.')
@click.option('--port', '-p', default=5432, help='The port to bind to. Default: 5432.')
@click.option('--debug/--no-debug', default=True, help='Enable or disable Flask debug mode. Default: False.')
