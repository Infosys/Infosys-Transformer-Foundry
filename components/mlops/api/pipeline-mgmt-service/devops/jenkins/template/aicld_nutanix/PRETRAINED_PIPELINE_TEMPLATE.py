...
def PIPELINE_NAME(RUN_ARG_NAME):
	...
	dsl.get_pipeline_conf().set_dns_config(dns_config=V1PodDNSConfig(nameservers=["192.168.200.56"],searches=["ad.infosys.com"]))
	...
	env_var8 = k8s_client.V1EnvVar(name='LOG_FILE_PATH', value='TRAIN_LOG_FILE_PATH')
	env_var9 = k8s_client.V1EnvVar(name='S3_LOG_FILE_PATH', value='PROJECT_ID/pipeline/PIPELINENAME/VERSION/runs/RUNNAME/outputartifacts/logs/')
	env_var10 = k8s_client.V1EnvVar(name='TF_LOG_FILE_PATH', value='s3://BUCKET_NAME/PROJECT_ID/pipeline/PIPELINENAME/VERSION/runs/RUNNAME/outputartifacts/tf_logs/')
	...
	storage_op.add_env_from(env_config_from)
	storage_op.add_env_variable(env_var1)
	storage_op.add_env_variable(env_var2)
	...
	log_stream_op.add_env_variable(env_var8)
	log_stream_op.add_env_variable(env_var9)
	...
	with dsl.ExitHandler(exit_task):
				...
				train.add_env_variable(env_var10)
