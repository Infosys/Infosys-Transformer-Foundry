...
"""
fileName: mappers.py
description: A Pydantic model object for benchmark entity model 
             which maps the data model to the benchmark entity schema
"""
...
class BenchmarkPipeline(BaseModel):
    ...
    def validateBenchmark(cls,values):
        ...
        if(not piplnnameEmptyErr and len(name)>int(BENCHMARK_PIPLN_NAME_MAX_LENGTH)):
            error=ValidationError(ErrorCode.BENCHMARK_PIPLN_NAME_MAX_LENGTH_ERROR)
            errors.append(error)
        ...
        if not configurationEmptyErr:
            ...
            if(not modelEmptyErr):
                ...
                for item in model:
                    ...
                    if (item.get('quantizeMethod').lower() not in ["static", "dynamic", "na"]):
                        error= ValidationError(ErrorCode.QUANTIZEMETHOD_VALUE_ERROR)
                        print(error)
                        errors.append(error)
                        quantizeMethodEmptyErr=True 
                    if (item.get('datatype').lower() in ["fp16","fp32"] and item.get('quantizeMethod').lower() not in ["na"]):
                        error= ValidationError(ErrorCode.QUANTIZEMETHOD_VALUE_ERROR_FP)
                        errors.append(error)
                    else:
