...
>(({ className, ...props }, ref) => {
  ...
  React.useEffect(() => {
    setTimeout(() => {
      ...
      if (thumbRef.current && thumbRef.current.parentElement) 
  ...
  >
    <SliderPrimitive.Track className="relative h-1 w-full grow overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
      <SliderPrimitive.Range className={`absolute h-full sliderBackground  dark:bg-slate-800 ${isDragging ? "" : "ease-in duration-500" }`} />
