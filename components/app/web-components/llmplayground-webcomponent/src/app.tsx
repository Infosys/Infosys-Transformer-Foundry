...
import PromptCompletionEditor from "./pages/playground"
  ...
  if (SETTINGS_SIZE >= 5) {
    ...
    if (shouldDownloadHistory) {
      ...
      const history_json = SETTINGS.pages["playground"].history.entries.map((entry: any) => {
        ...
        // console.log(`Model in request - ${model}`);
        // alert(`Model in request - ${model}`);
...
const APIContextWrapper = ({children}) => {
  ...
  useEffect(() => {
    const sse_request = new SSE("http://localhost:30008/plygrdapp/api/notifications")
  ...
  const Model = {
    // getAll: async () => (await fetch("http://10.87.26.11:32667/plygrdapp/api/models")).json(),
    getAll: async () => (await fetch("http://localhost:30008/plygrdapp/api/models")).json(),
    getAllEnabled: async () => (await fetch("http://localhost:30008/plygrdapp/api/models-enabled")).json(),
    toggle: async (provider, model) => (await fetch(`http://localhost:30008/plygrdapp/api/provider/${provider}/model/${encodeURIComponent(model)}/toggle-status`)).json(),
    search: async (provider, query) => (await fetch(`http://localhost:30008/plygrdapp/api/provider/${provider}/models/search?query=${query}`)).json(),
  };
  ...
  const Provider = {
    setAPIKey: async (provider, apiKey) => (await fetch(`http://localhost:5432/plygrdapp/api/provider/${provider}/api-key`, {method: "PUT", headers: {"Content-Type": "application/json"}, 
      body: JSON.stringify({apiKey: apiKey})}
    )).json(),
    getAll: async () => (await fetch("http://localhost:5432/plygrdapp/api/providers-list")).json(),
    getAllWithModels: async () => (await fetch("http://localhost:5432/plygrdapp/api/providers-with-key-and-models")).json(),
  };
  ...
  function createTextCompletionRequest({prompt, models}) {
    const url = "http://localhost:5432/plygrdapp/api/inference/text/stream";
    // add userID to each openAI model
    const userID = dataStorageService.getData().userId;
    models = models.map((model) => {
      if (model.provider === "openai") {
        model.parameters["user-Id"] = userID || "user";
      }
      return model;
    });
...
const PlaygroundContextWrapper = ({page, children}) => {
  ...
  const updateModelsData = async () => {
    ...
    for (const [model_key, modelDetails] of Object.entries(json_params)) {
      ...
      console.log("ModelDetails",model_key, modelDetails);
      ...
      if (!existingModelEntry) {
        PAGE_MODELS_STATE.push({
          ...
          selected: false,
          tasktype: modelDetails.tasktype,
          endpoint: modelDetails.endpoint,
          metadata: Object.entries(modelDetails.metadata).reduce((acc, [key, field]) => {
            acc[key] = field || "abcd"; 
            return acc;
          }, {}),
        })
        ...
        if (!existingModelEntry.parameters) {
          ...
          existingModelEntry.tasktype = modelDetails.tasktype,
          existingModelEntry.endpoint = modelDetails.endpoint,
          existingModelEntry.parameters = Object.entries(modelDetails.parameters).reduce((acc, [key, fields]) => {
            acc[key] = fields.value;
            return acc;
          }, {}),
          existingModelEntry.metadata = Object.entries(modelDetails.metadata).reduce((acc, [key, fields]) => {
            acc[key] = fields.value;
            return acc;
          }, {})      
        }
...
<Route path="/promptlibrary" element={<PromptLibrary />} />
    ...
    /* <Route path="/promptlibrary" 
element={ <APIContextWrapper>
<PromptLibrary/>
</APIContextWrapper>} /> */
...
export default function App({ userID }: AppProps) {
  console.log("appcomponet with userid", userID);
    return (
