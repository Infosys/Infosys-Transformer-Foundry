...
const ModelCardStats = (props: any) => {
  ...
  return (
    <div className="flex" style={{ fontSize: 'small', color: 'black'}}>
...
function PromptEditor({editorState, setEditorState, ...props}: any) {
  ...
  const modelsEnabled = number_of_models_enabled === 0 || number_of_models_enabled > 3
  return (
    <div
      ref={scrollRef}
      onClick={focusEditor}
    ...
    >
      <Editor
        placeholder={
          modelsEnabled ? "Select atleast 1 model and maximum 3 models..."
...
function PromptArea({showDialog}) {
  ...
  return (
    ...
    style={{display:"flex", alignSelf:"flex-end", position:"sticky"}}>
          {generating && (
          ...
          <>
            <Tooltip delayDuration={100}>
              <TooltipTrigger asChild>
                <Button
                  type="button"
                  className="bg-emerald-500 tf_button inline-flex items-center ml-1 text-sm font-medium text-center"
                  disabled={editorContext.prePrompt === ""}
                  onClick={() => {
                    resetEditor()
                  }}
                  style={{ position: "sticky", alignSelf: "flex-end" }}
                >
                  Clear
                </Button>
              </TooltipTrigger>
            ...
            <Tooltip delayDuration={100}>
              <TooltipTrigger asChild>
                <Button
                  disabled={number_of_models_enabled === 0 || number_of_models_enabled > 3}
                  variant="default"
                  className="bg-emerald-500 tf_button inline-flex items-center ml-1 text-sm font-medium text-center"
                  type="submit"
                  value="submit"
...
const ModelsCompletion = ({ showDialog }) => {
  ...
  const [signal, signalRender] = React.useState(0);
  ...
  // Log state changes
  useEffect(() => {
    console.log('modelsCompletionState changed:', modelsCompletionState);
  }, [modelsCompletionState]);

  // Log signal state changes
  useEffect(() => {
    console.log('signal changed:', signal);
  }, [signal]);

  useEffect(() => {
    console.log('number_of_models_enabled changed:', number_of_models_enabled);
  }, [number_of_models_enabled]);
    ...
    const completion_callback = ({ event, data }) => {
      switch (event) {
        ...
        case "completion":
          ...
          console.log("modelState before", modelsCompletionState);
          
          setModelsCompletionState((prevState) => {
          ...
          signalRender((x) => x + 1);
          console.log("modelState after", modelsCompletionState);

          console.log("Completion", signal);
...
const ParametersTable = () => {
  ...
  const generate_row = (modelState, model) => (
    [
      "name", "max_new_tokens", "temperature", "topP", "topK", "frequencyPenalty", "presencePenalty", "repetitionPenalty"
    ].map((parameter, index) => (
      (parameter === "name") ?
  ...
  return (
    <div className="flex flex-col h-full" style={{ height: height, backgroundColor: 'GhostWhite' }}>
      ...
      <div className="flex flex-grow flex-col font-display min-h-0 min-w-0" style={{ background: 'white' }}>
        <div className="flex flex-row space-x-4 flex-grow mx-2 md:ml-5 lg:ml-5 min-h-0 min-w-0" style={{ margin: '10px' }}>
          ...
          <div className="hidden p-1 grow-0 shrink-0 basis-auto lg:w-[250px] overflow-auto lg:block"
            style={{ marginLeft: '8px', paddingLeft: '10px', paddingTop: '0px' }}>
            {parametersSidePanel}
          </div>
        </div>
      </div>
