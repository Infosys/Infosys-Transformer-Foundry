...
export class ModelHomeComponent implements OnInit {
  ...
  // check if the current page is model zoo
  checkIfModelZoo() {
  ...
  private setQueryParams(){
    ...
    parent.route.queryParamMap.subscribe(queryParams => {
      ...
      let newParams:any = JSON.stringify(queryParams['params']);
      let keysToRemove = ['bm', 'mdl','td','fd'];
      for (const key of keysToRemove) {
        const regex = new RegExp(`,"${key}":"[^"]*"`);
        newParams = newParams.replace(regex, '');
      }
      newParams = JSON.parse(newParams);
      console.log("params:",newParams);
      parent.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
